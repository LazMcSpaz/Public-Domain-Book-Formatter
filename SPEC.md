...
- **Book covers** — out of scope. *But the tool must report final page count, which the user needs for spine-width math.*
...
- **Scroll-sync:** scrolling one pane tracks the other, so a long read never drifts apart. *As important as hover-sync for whole-book reading.*
...
- **Source-image-on-hover** — show the actual cropped scan pixels of a word/region. For ambiguous OCR, the original pixels resolve "is that an l or a 1" better than text-to-text. *Core for a scan workflow.*
...
- **Footnote tag** does two jobs: pulls the text out of the body flow *and* re-links it to its in-text reference mark, so XeLaTeX sets it properly at the page bottom. Replaces the cut-and-paste-elsewhere pain with one click.
...
Automated extraction from old scans is **unreliable** — so the design treats auto-extraction as a *first guess* and makes manual editing the real tool.
...
- **Saved style profile** — the reusable *look*, divorced from content: trim size, margins, gutter rules, body font/size, heading fonts/styling, running-head layout, page-number treatment, ornament choices and placement, front-matter visual design. Banked once, applied across books.
...
- **Table of contents** — **auto-generated** from detected/confirmed chapter headings, with correct *edition* page numbers. The original scanned TOC (with wrong page numbers) is discarded.
...
OCR and formatting *feel* hard but are largely tool-solved. The labor that actually fills time is structural:
...
3. OCR via Tesseract/OCRmyPDF → text **with hOCR confidence + bounding boxes**. *(Backbone — build the coordinate mapping here.)*
