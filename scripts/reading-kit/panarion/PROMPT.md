...
- **Spacing before `;` `:` `?` `!`**: the compositor sets a thin space there nearly everywhere, narrower than a word space. **Any visible gap is set as one space; only a mark touching its word is set close** (`serious?`). Inside quotation marks it is different in this book: the `“` and `”` carry a wide sidebearing of their own, so set them close unless the gap is clearly wider than letter spacing (`Bravery ”` spaced; most close). Do not normalise, in either direction, what you can see.
