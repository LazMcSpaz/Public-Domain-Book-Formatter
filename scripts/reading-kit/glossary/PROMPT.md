...
- A worn `e` is not the same as a lightly inked one: where the crossbar is faint but present it is `e`, and only a letter that plainly prints `c` is queried. A worn circumflex prints as a grave-like tick in **roman** too (`Kâlî`, `Vedânta`), not only in italic.
- Greek is printed without breathings or accents in most entries — transcribe it bare, with the final sigma the page sets; where the page does print accents (`ἠώς`), keep them. Both engines drop Greek entirely.
